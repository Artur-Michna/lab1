...
public:
	...
	void Draw() const {
		...
		case WeaponType::PLASMA:
			DrawCircleV(transform.position, 8.f, PURPLE);
			break;
		default:
			break;
		}
	...
	float GetRadius() const {
		...
		case WeaponType::PLASMA: return 8.f;
		default: return 5.f;
		}
	}
	int GetDamage() const {
	...
	float speed)
{
	...
	case WeaponType::PLASMA:
		return Projectile(pos, vel, 30, wt);  // PLASMAAAAAAAAAAAAAAAAAAAAAAAAA
	default:
		return Projectile(pos, vel, 0, wt);  // difult
	}
}
...
public:
	...
	float GetFireRate(WeaponType wt) const {
		...
		case WeaponType::PLASMA: return fireRatePlasma;
		default: return 0.f;
		}
	}
	...
	float GetSpacing(WeaponType wt) const {
		...
		case WeaponType::PLASMA: return spacingPlasma;
		default: return 0.f;
		}
	}
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						...
						DrawCircleV((*ait)->GetPosition(), 20.0f, ORANGE);
						removed = true;
						break;
					}
				}
			...
			// Render everything
			{
				...
				if (!player->IsAlive()) {
					DrawText("Game over! press R to try again", C_WIDTH / 2 - 150, C_HEIGHT / 2, 20, RED);
				}
